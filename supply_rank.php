...
/*输出数据语句*/
$query = "select wu.id,wu.weixin_headimgurl,wu.name,wu.weixin_name,wca.charitable,wu.charitable_level from weixin_commonshop_applysupplys wca inner join weixin_users wu on wca.user_id = wu.id where wca.isvalid=true and wca.status=1 and wu.customer_id=".$customer_id;
/*输出数据语句*/
/*统计数据数量*/
$query_num ="select count(1) as wcount from weixin_commonshop_applysupplys wca inner join weixin_users wu on wca.user_id = wu.id where wca.isvalid=true and wca.status=1 and wu.customer_id=".$customer_id;
/*统计数据数量*/
$sql = "";
 if(!empty($search_id)){			   
	$sql .= " and wca.user_id like '%".$search_id."%'";
}
				...
				<form class="search" id="search_form" style="margin-left:18px; margin-top: 18px;">
					<div class="WSY_list" style="margin-top: 18px;">
						<li class="WSY_position_text">
							<a>供应商编号：<input type="text" name="search_id" id="search_id" value="<?php echo $search_id; ?>"></a>
					...
					<tbody>
					   ...
					   ?>
						<tr>
							...
							<td>
								<img style="height:60px;" src="<?php echo $weixin_headimgurl; ?>">
							</td>
							<td>
								<a href="supply_detail.php?customer_id=<?php echo $customer_id_en; ?>&user_id=<?php echo $user_id; ?>">
									<?php echo $username; ?>
								</a>
							</td>
...
<script type="text/javascript" src="../../Common/js/Mode/charitable/supply_rank.js"></script>
</body>
</html>